...
use crate::prelude::*;
use layer_climb::{prelude::*, web::KeplrSigner};
...
pub async fn client_connect(key_kind: ClientKeyKind, target_env: TargetEnvironment) -> Result<()> {
    let chain_config = match target_env {
        TargetEnvironment::Testnet => {
            CONFIG.data.chains.testnet.clone().context("testnet chain not configured")?
        },
        TargetEnvironment::Local => {
            CONFIG.data.chains.local.clone().context("local chain not configured")?
        }
    };
