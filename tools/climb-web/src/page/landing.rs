use crate::{client::{add_keplr_chain, client_connect, ClientKeyKind, TargetEnvironment}, prelude::*};
...
impl LandingUi {
    pub fn new() -> Arc<Self> {
        Arc::new(Self {
            ...
            client_key_kind: Arc::new(Mutex::new(CONFIG.debug.auto_connect.as_ref().map(|x| x.key_kind.clone()))),
            target_environment: Mutable::new(CONFIG.debug.auto_connect.as_ref().map(|x| x.target_env.clone())),
