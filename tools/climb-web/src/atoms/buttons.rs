...
impl Button {
    ...
    pub fn with_disabled_signal(mut self, disabled_signal: impl Signal<Item = bool> + 'static) -> Self {
    ...
    pub fn render(self) -> Dom {
        ...
        let Self { size, color, text, disabled_signal: disabled_sig, mut on_click, style, mixin } = self;
        ...
        let neither_hover_nor_disabled_signal = || map_ref! {
        ...
        let hover_but_not_disabled_signal = || map_ref! {
            let disabled = disabled.signal(), 
            let hovering = hovering.signal() => {
                !*disabled && *hovering
            }
